...
bp::object PyHandle::ExecFile(const char* file)
{
	...
	Q_snprintf( fullPath, sizeof(fullPath), "%s\\%s", fname, pf );
	
	//filesystem->RelativePathToFullPath( pf, "GAME", fullPath, 255);
...
void CGEPYManager::InitDll()
{
	...
	try
	{
		...
		char fname[100]; 
		GetCurrentDirectory(100, fname);
		Q_snprintf( filePath, sizeof(filePath), "%s\\%s", fname, GetRootPath() );
		
		Msg("[EnginePython] Python Home Path: ");
		Msg(filePath);
		Msg("\n");
		//filesystem->RelativePathToFullPath(GetRootPath(), "GAME", filePath, sizeof(filePath) );
...
bp::object CGEPYManager::ExecFile(const char* name)
{
	...
	char fname[100]; 
	GetCurrentDirectory(100, fname);
	Q_snprintf( fullPath, sizeof(fullPath), "%s\\%s", fname, file );
	
	//filesystem->RelativePathToFullPath( file, "GAME", fullPath, 255);
...
CON_COMMAND(py, "[EnginePython] Exec python")
{
	if ( args.ArgC() < 2 )
	{
...
CON_COMMAND(exec_py, "[EnginePython] Exec python file")
{
	if ( args.ArgC() < 2 )
	{
		Msg("Executes python file\n");
		return;
	}
